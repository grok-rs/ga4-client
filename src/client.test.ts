...
describe('GA4Client', () => {
  ...
  describe('sendWithRetry', () => {
    it('should retry on server error', async () => {
      ...
      const client = new GA4Client({ ...defaultOptions, fetch: mockFetch as unknown as typeof fetch });
      await client.sendWithRetry([{ client_id: 'test', events: [{ name: 'test' }] }], { maxRetries: 3, initialDelayMs: 1 });
    ...
    it('should not retry on client error', async () => {
      ...
      await assert.rejects(client.sendWithRetry([{ client_id: 'test', events: [{ name: 'test' }] }], { maxRetries: 3 }), GA4Error);
      assert.equal(mockFetch.mock.callCount(), 1);
    });
  });
