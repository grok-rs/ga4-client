...
export class GA4Client {
  ...
  async sendWithRetry(events: GA4Event[], options: RetryOptions = {}): Promise<void> {
    const { maxRetries = GA4.MAX_RETRIES, initialDelayMs = GA4.INITIAL_RETRY_DELAY_MS, maxDelayMs = GA4.MAX_RETRY_DELAY_MS } = options;
  ...
  private async post(payload: GA4Event, debug: boolean): Promise<Response> {
    ...
    try {
      ...
      throw new GA4Error(GA4ErrorCode.Request, isTimeout ? `Timeout after ${this.config.timeoutMs}ms` : `Request failed: ${msg}`);
    } finally {
